...
- Jēkabs Čudars ([cudars@kth.se](mailto:cudars@kth.se))
...
2. **Example project repository**
   ...
   * Contains the release workflow:
     ...
     - **Create Signed Tag:** runs `git cliff bump` to determine the next version and creates and pushes a cryptographically signed Git tag.
     - **Create GitLab Release:** creates a GitLab Release associated with the signed tag and uses the generated changelog as the release notes.
     - The release workflow will ensure that published releases cannot be modified after creation where GitLab's release protection mechanisms support this.

The tutorial will demonstrate the complete workflow from a new commit to an automatically generated changelog, signed version tag, and GitLab Release.
