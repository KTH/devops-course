...
- **Automated build and testing (CI)**: GitHub Actions workflow that lints, runs unit tests (pytest) and builds a Docker image on every push/PR.
...
- **Quality/security automation**: Dependabot for dependency updates, CodeQL static analysis, and a container image scan (e.g., Trivy) in the CI pipeline; secrets (bot token, API keys) are managed via GitHub Actions secrets and never committed.
...
| Scheduling | APScheduler (periodic price polling) |
| Market data | CoinGecko API (crypto), Alpha Vantage or Yahoo Finance API (stocks) |
| Persistence | SQLite (or PostgreSQL) for subscriptions and alert state |
| Containerization | Docker |
| CI | GitHub Actions (lint, pytest, Docker build) |
| CD | GitHub Actions → GHCR → deploy to VM/container host |
| Infrastructure as Code | Terraform (provisioning) + Ansible or Docker Compose (configuration) |
| Quality/security automation | Dependabot, CodeQL, Trivy container scan |
